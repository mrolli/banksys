...
public class WithdrawMoneyAction extends FormAction {
	...
	private static final String WITHDRAW_MESSAGE ="Schalterbezug";
			...
			try {
				...
				if (moneyValue > 0) {
					...
					if (Session.getInstance().isOwnCustomer()) {
						transactionNumber = CounterServiceProxy.getInstance().withdrawMoney(iban, amount, WITHDRAW_MESSAGE);
